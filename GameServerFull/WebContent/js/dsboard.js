/*!
 * dsboard,js SWTPP team (fellmuth)
 * 
 * adapted from:
...
function validSquare(square) {
  ...
  return (square.search(/^[a-f][1-6]+$/) !== -1);
}

function validMoveNumber(square) {
	  if (typeof square !== 'string') return false;
	  return (square.search(/^[0-9]+$/) !== -1);
	}
...
function validFen(fen) {
  ...
  for (var i = 0; i < BOARD_SIZE; i++) {
	...
	if (chunks[i].search(/[^,rb]/) !== -1) {
	      return false;
	}

	var figs = chunks[i].split(',');
	
	if (figs.length > BOARD_SIZE || chunks[i].search(/[^,rb]/) !== -1) {
	      return false;
	    }
...
window['DeathStacksBoard'] = window['DeathStacksBoard'] || function(containerElOrId, cfg) {
'use strict';
...
var MINIMUM_JQUERY_VERSION = '1.7.0',
  START_FEN = 'rr,rr,rr,rr,rr,rr/,,,,,/,,,,,/,,,,,/,,,,,/bb,bb,bb,bb,bb,bb',
...
function checkDeps() {
  ...
  if (typeof containerElOrId === 'string') {
    ...
    if (containerElOrId === '') {
      window.alert('Board Error 1001: ' +
        'The first argument to DeathStacksBoard() cannot be an empty string.' +
        '\n\nExiting...');
      return false;
    }
  ...
  // ie: a single DOM node or jQuery object
    ...
    if (containerEl.length !== 1) {
      window.alert('Board Error 1003: The first argument to ' +
        'DeathStacksBoard() must be an ID or a single DOM node.' +
        '\n\nExiting...');
      return false;
    }
  }
  ...
  if (! window.JSON ||
    ...
    window.alert('DeathStacksBoard Error 1004: JSON does not exist. ' +
  ...
  if (! (typeof window.$ && $.fn && $.fn.jquery &&
    ...
    window.alert('DeathStacksBoard Error 1005: Unable to find a valid version ' +
...
function paintChip(canvas, offset, red) {
...
function buildPieceImgSrc(piece) {
	...
	for(var i=pieces.length-1; i >= 0; i--) {
		...
		switch(pieces[i]) {
			case 'r': paintChip(canvas, offset, true); break;
			case 'b': paintChip(canvas, offset, false); break;
			default: 
		}
...
}; // end window.DeathStacksBoard

// expose util functions
window.DeathStacksBoard.fenToObj = fenToObj;
window.DeathStacksBoard.objToFen = objToFen;
